    ...
    def motion(self):
        ...
        if self.mode == 'keyboard':
            ...
            if data == b'u':
                ...
                gbvn = data.decode('utf8')
                try:
                    if gbvn[0] == 'b':
                        ball_angle = float(gbvn[1:])
                    elif gbvn[1] == 'b':
                        ball_angle = float(gbvn[2:])
                except IndexError:
                    pass
        if self.y1 < 0:
            self.y1 += self.movement
...
def regen_ball():
    ...
    del ball
    client_sock.sendall(b'0')
    ba = client_sock.recv(1024).decode('utf8')
    try:
        if ba[0] == 'b':
            _ball_angle = float(ba[1:])
        elif ba[1] == 'b':
            _ball_angle = float(ba[2:])
    except IndexError:
        _ball_angle = ball_angle
    ball = Ball(_ball_angle)
    objects.append(ball)
...
pygame.display.set_caption('Internet Pong v1.0 (player two)')
cmd = ''
while cmd != b's': # пока первая программа не дала комманду "Старт" (т.е. b's'), это программа не начнёт работу.
    cmd = client_sock.recv(1024)
else:
    client_sock.sendall(b's')
ba = client_sock.recv(1024).decode('utf8')
if ba[0] == 'b':
    ball_angle = float(ba[1:])
ball = Ball(ball_angle)
objects = [pdl2, pdl, ball]
rects = [pdl2, pdl]
while True:
    ...
    pygame.display.flip()
    if score_left == 5:
        print('Player one wins! That\'s not me...')
        exit()
    elif score_right == 5:
        print('Player two wins! That\'s me!')
        exit()